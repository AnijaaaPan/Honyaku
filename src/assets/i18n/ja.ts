const ja = {
  commands: {
    advisor: {
      delete: {
        completed: '>>> **※削除が完了しました**',
        description: 'アドバイザー役職を削除する'
      },
      description: 'アドバイザー関連',
      list: {
        description: 'アドバイザー役職一覧を表示します'
      },
      set: {
        completed: '>>> **※アドバイザー役職の作成が完了しました**',
        contentYesNo: '**アドバイザー役職を作成しますか？**',
        description: 'アドバイザー役職を作成します',
        embed: {
          description: '**{0}: {1} リアクションポイント**'
        },
        hasSamePointData: '>>> **※同じポイントを持つ役職が既に設定されています**'
      }
    },
    autoVc: {
      channel: {
  ...
  rolePannel: {
    addMessage: '>>> **※<@&{0}>を付与しました**',
    removeMessage: '>>> **※<@&{0}>を外しました**',
    undefinedGetRoleId: '>>> **※想定外のCustomIDが処理されています**'
  },
  undefinedData: '>>> **※データがありませんでした**'
} as const
